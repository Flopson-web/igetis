        ...
        <form method="POST" action="{{ route('admin.cursos.update', $curso) }}" enctype="multipart/form-data"
            ...
            <div style="margin-bottom:1.25rem;">
                ...
                <textarea name="objetivos" rows="4"
                          style="width:100%; padding:0.625rem 0.875rem; border:1px solid #d1d5db; border-radius:0.5rem; font-size:0.875rem; outline:none; resize:vertical;">{{ old('objetivos', $curso->objetivos) }}</textarea>
            </div>
            ...
            <div style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:1rem; margin-bottom:1.25rem;">
                ...
                <div>
                    ...
                    <input type="text" name="duracion" value="{{ old('duracion', $curso->duracion) }}" placeholder="Ej: 40 horas"
                           style="width:100%; padding:0.625rem 0.875rem; border:1px solid #d1d5db; border-radius:0.5rem; font-size:0.875rem; outline:none;">
                </div>
                <div>
                    <label style="display:block; font-size:0.875rem; font-weight:600; color:#374151; margin-bottom:0.375rem;">
                    ...
                    <select name="modalidad"
                        ...
                        @foreach (['Presencial', 'Virtual', 'Híbrida'] as $opcion)
                            <option value="{{ $opcion }}" {{ old('modalidad', $curso->modalidad) === $opcion ? 'selected' : '' }}>
                                {{ $opcion }}
                            </option>
                        @endforeach
                    </select>
                ...
                @if ($curso->imagen)
                    <div style="margin-bottom:0.5rem;">
                        <img src="{{ Storage::url($curso->imagen) }}" alt="Imagen actual"
                             style="height:80px; border-radius:0.375rem; object-fit:cover;">
                        <p style="font-size:0.75rem; color:#6b7280; margin:0.25rem 0 0;">Imagen actual. Sube una nueva para reemplazarla.</p>
                    </div>
                @endif
